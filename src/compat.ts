...
/** Validate that an s402 object has all required fields with correct types. */
function validateS402Shape(obj: Record<string, unknown>): void {
  const missing: string[] = [];
  if (!Array.isArray(obj.accepts)) missing.push('accepts (array)');
  if (typeof obj.network !== 'string') missing.push('network (string)');
  if (typeof obj.asset !== 'string') missing.push('asset (string)');
  if (typeof obj.amount !== 'string') missing.push('amount (string)');
  if (typeof obj.payTo !== 'string') missing.push('payTo (string)');
  if (missing.length > 0) {
    throw new s402Error('INVALID_PAYLOAD',
      `Malformed s402 requirements: missing or invalid fields: ${missing.join(', ')}`);
  }
}
