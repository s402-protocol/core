...
**Programmable Transaction Blocks (PTBs).** A PTB is a single atomic transaction that encodes multiple operations — reads, writes, condition checks — that either all succeed or all fail together. For payment protocols, this eliminates the most dangerous gap in settlement: the window between "payment verified" and "payment settled." On EVM, the stateless single-signature flow leaves this gap open — verification is a read (off-chain simulation), settlement is a write (on-chain broadcast), and a stateless facilitator can be raced between them: measured in production at 6% duplicate settlement under concurrency, with up-to-99× allowance overdraft on `upto`-style schemes (Ling et al., arXiv:2605.30998, ACM ATC '26). EVM can close the gap, but only by pre-funding an on-chain escrow or payment channel (x402's `auth-capture` and `batch-settlement` schemes) or by trusting a stateful facilitator to linearize settlement. On Sui, verification and settlement execute atomically in one PTB — no pre-funded escrow, no stateful linearizer. Either the payment goes through or it doesn't, with no intermediate state.
