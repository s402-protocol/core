...
describe('s402 errors', () => {
  ...
  it('retryable codes are correctly marked', () => {
    const retryable = ['STREAM_DEPLETED', 'UNLOCK_DECRYPTION_FAILED', 'FINALITY_TIMEOUT', 'FACILITATOR_UNAVAILABLE', 'REQUIREMENTS_EXPIRED', 'SETTLEMENT_FAILED'];
    const nonRetryable = ['INSUFFICIENT_BALANCE', 'MANDATE_EXPIRED', 'MANDATE_LIMIT_EXCEEDED', 'ESCROW_DEADLINE_PASSED', 'INVALID_PAYLOAD', 'SCHEME_NOT_SUPPORTED', 'NETWORK_MISMATCH', 'SIGNATURE_INVALID', 'VERIFICATION_FAILED', 'DIGEST_MISMATCH', 'EXTENSION_FAILED'];
